...
public class AutoAimRotate extends CommandBase {
  ...
  Integer m_missCounter = 0;

  public AutoAimRotate(SwerveSubsystem swerve, PhotonVision PhotonVision, Boolean shooter, ControlScheme scheme) {
    m_swerve = swerve;
  ...
  public void execute() {
    ...
    if (result.hasTargets()) {
      ...
      m_missCounter += 1;
      var joystick = m_scheme.getJoystickSpeeds();
      if (m_missCounter >= 10) {
        // Override with a constant rotation
        joystick.m_rotation = AUTO.AIMROTATE;
      }
      m_swerve.dt.setModuleStates(joystick);
    }
  }
